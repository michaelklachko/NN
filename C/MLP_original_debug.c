...
int dot(float** array1, float** array2, float** result, int dim1, int dim2, int dim3, int debug)
{
	...
	for(k=0; k<dim3; k++) {
	    printf("k=%d, ", k);
	    printf("result[i][k] = %.2f. ", result[i][k]);
	}
    }
		...
		int batch_size, int n_hidden, struct params* p)
{
    ...
    dot(batch, p->W1, z_hidden, batch_size, img_size, n_hidden, 0);

    add_vec(z_hidden, p->b1, batch_size, n_hidden);

    ReLU_vec(z_hidden, output_hidden, batch_size, n_hidden);

    dot(output_hidden, p->W2, z_out, batch_size, n_hidden, n_out, 0);
	    ...
	    float** batch_transposed)
{
    ...
    printf("\n2\n");
    dot(output_hidden_transposed, error_out, grad->W2, n_hidden, batch_size, n_out, 0);
    
    printf("\n3\n");
    transpose(W2, W2_transposed, n_hidden, n_out);

    printf("\n4\n");
    dot(error_out, W2_transposed, error_hidden, batch_size, n_out, n_hidden, 0);
    ...
    */
    ...
    printf("\n9\n");
    dot(batch_transposed, error_hidden, grad->W1, img_size, batch_size, n_hidden, 0);
